...
# Add a slider for price range
price_min, price_max = st.sidebar.slider(
    "Select Consultation Fee Range (in ₹):",
    min_value=0,
    max_value=5000,
    ...
    if st.sidebar.button("Search"):
        if location_input and specialization_input:
            with st.spinner("Fetching data..."):
                ...
                # Filter profiles based on the selected price range
                filtered_profiles = [
                    profile for profile in profiles
                    if profile['consultation_fees'] != "N/A" and
                    price_min <= int(profile['consultation_fees'].replace("₹", "").strip()) <= price_max
                ]
                ...
                if filtered_profiles:
                    ...
                    st.warning("No doctor profiles match the selected price range.")
        else:
